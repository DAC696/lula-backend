  ...
  try {
    ...
    if (parts && Array.isArray(parts) && parts.length)
      for (let part of parts) {
        ...
        const updatedPart = await PartModel.updatePartById(partId, { quantity: minusQuantity })
  ...
  try {
    ...
    if (status !== undefined) {
      ...
      if (status == 'completed') {
        ...
        let partsArr = []
        if (workOrder.parts && Array.isArray(workOrder.parts) && workOrder.parts.length)
          for (let part of workOrder.parts) {
            ...
            const updatedPart = await PartModel.updatePartById(partId._id, { quantity: minusQuantity })
    ...
    const updatedWorkOrder = await WorkOrderModel.updateWorkOrderById(workOrderId, updateData);
...
const deleteWorkOrderById = async (req, res, next) => {
  try {
    ...
    const deletedWorkOrderCount = await WorkOrderModel.deleteWorkOrderById(workOrderId);
